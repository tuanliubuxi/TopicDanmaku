...
sidebar.transparent_bac.visible = false;	//隐藏透明背景
...
function menu_io(e:MouseEvent):void
{
	...
	if(sidebar.x == -sidebar.main_bac.width)
	{
		sidebar.transparent_bac.visible = true;	//显示透明背景
	}
	else
	{
		sidebar.transparent_bac.visible = false;	//隐藏透明背景
...
sidebar.transparent_bac.addEventListener(MouseEvent.MOUSE_DOWN, hideMenu);
function hideMenu(e:MouseEvent):void
{
	if(sidebar.x == -sidebar.main_bac.width) return;	//如果菜单栏未打开则不执行
	menu_io(e);	//点击透明背景时收起菜单栏
}
